...
module Lib
  module TL1
    module Huawei
      module Message
        # Queries the dynamic bandwidth allocation profile associated with an ONT.
        class LstOntDbaProf < Lib::TL1::Huawei::Message::Input
          ...
          def initialize(
            ...
            set_attributes(
              did:, dev:, frame_number:, slot_number:, port_number:, ont_id:, ont_pwd:, show_option:
            )
            super(
              aid: hash_to_string(
                did: did, dev: dev, fn: frame_number, sn: slot_number, pn: port_number, ont_id: ont_id, pwd: ont_pwd
              ),
